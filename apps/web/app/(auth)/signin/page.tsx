...
export default function SignInPage() {
  ...
  async function handleEmailAuth(e: React.FormEvent) {
    ...
    try {
      ...
      const body = mode === 'signin' ? { email, password } : { email, password, displayName: email.split('@')[0] };
      ...
      if (!res.ok || !json.ok) {
        throw new Error(json?.error?.message ?? 'حدث خطأ أثناء العملية');
      }
      
      if (mode === 'signin') {
        window.location.href = json.data?.redirectTo ?? '/app';
      } else {
        setError('✅ تم تسجيل الحساب بنجاح! يمكنك الآن تسجيل الدخول.');
        setMode('signin');
      }
    } catch (err: any) {
  ...
  async function handleOAuth(provider: 'google' | 'github') {
    ...
    setError(null);
    window.location.href = `/api/v1/auth/oauth/request?provider=${provider}`;
  }
