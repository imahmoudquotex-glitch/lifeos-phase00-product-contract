...
describe('assertCsrf', () => {
    it('throws CSRF_TOKEN_MISSING when tokens are empty', () => {
        expect(() => assertCsrf({ cookieToken: '', bodyToken: 'x' })).toThrow(expect.objectContaining({ code: 'CSRF_TOKEN_MISSING' }));
    });
    it('throws CSRF_TOKEN_INVALID for mismatch', () => {
        expect(() => assertCsrf({ cookieToken: 'aaa', bodyToken: 'bbb' })).toThrow(expect.objectContaining({ code: 'CSRF_TOKEN_INVALID' }));
    });
