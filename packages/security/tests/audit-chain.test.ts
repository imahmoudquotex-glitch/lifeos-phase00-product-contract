...
const BASE_EVENT: AuditEventInput = {
	workspaceId: 'ws_1',
	actorId: 'u_1',
...
describe('verifyChain', () => {
	...
	it('returns false for tampered payload (DB hash does not match recomputed)', () => {
		const h = computeEventHash(GENESIS_HASH, BASE_EVENT);
		const tampered: StoredAuditEvent = {
			...BASE_EVENT,
			id: 'e_1',
			prevHash: GENESIS_HASH,
			eventHash: h,
	...
	it('returns true for valid 2-event chain', () => {
		...
		const ev1: StoredAuditEvent = { ...BASE_EVENT, id: 'e_1', prevHash: GENESIS_HASH, eventHash: h1 };
		const ev2Input: AuditEventInput = { ...BASE_EVENT, eventType: 'task:updated' };
		const h2 = computeEventHash(h1, ev2Input);
		const ev2: StoredAuditEvent = { ...ev2Input, id: 'e_2', prevHash: h1, eventHash: h2 };
...
describe('assertChain', () => {
	it('throws AUDIT_CHAIN_BROKEN for broken chain', () => {
		...
		expect(() => assertChain([ev])).toThrow(
			expect.objectContaining({ code: 'AUDIT_CHAIN_BROKEN' }),
		);
	});
});
