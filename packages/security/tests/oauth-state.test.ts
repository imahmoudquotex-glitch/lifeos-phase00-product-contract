...
describe('assertOAuthState', () => {
	it('throws OAUTH_STATE_INVALID when stored is null', () => {
		expect(() => assertOAuthState(null, 'state', new Date())).toThrow(
	...
	it('throws OAUTH_STATE_EXPIRED when expired', () => {
		...
		expect(() => assertOAuthState(stored, 'abc', new Date('2026-01-01T00:00:00Z'))).toThrow(
	...
	it('throws OAUTH_STATE_INVALID on mismatch', () => {
		...
		expect(() => assertOAuthState(stored, 'xyz', new Date())).toThrow(
			expect.objectContaining({ code: 'OAUTH_STATE_INVALID' }),
		);
	});
