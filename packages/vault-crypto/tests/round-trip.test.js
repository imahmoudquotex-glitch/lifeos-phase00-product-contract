...
describe('Full vault round-trip', () => {
    it('create → lock → unlock → seal → open', async () => {
        ...
        expect(Buffer.from(unlockedMaster).toString('hex')).toBe(Buffer.from(masterKey).toString('hex'));
        // 4. Unwrap item key
        const recoveredItemKey = unwrapItemKey(unlockedMaster, new Uint8Array(wrapped), new Uint8Array(wrapNonce));
        expect(Buffer.from(recoveredItemKey).toString('hex')).toBe(Buffer.from(itemKey).toString('hex'));
